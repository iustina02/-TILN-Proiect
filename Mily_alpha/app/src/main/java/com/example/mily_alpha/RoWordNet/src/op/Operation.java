package com.example.mily_alpha.RoWordNet.src.op;
...
public class Operation {
	...
	public static String[] diff(RoWordNet a, RoWordNet b) {
		...
		while (it.hasNext()) {
			Entry<String, Synset> entry = (Entry<String, Synset>) it
					.next();
	...
	public static RoWordNet merge(RoWordNet base, RoWordNet addition) {
		Iterator<Entry<String, Synset>> it = addition.synsetsMap.entrySet()
				.iterator();
		while (it.hasNext())
			base.addSynset(((Entry<String, Synset>) it.next()).getValue(), true);
